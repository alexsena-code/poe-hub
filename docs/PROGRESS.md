...
Last updated: 2026-08-21 (session 26 — nova tela `/farm/bot-sales` para simular venda de bots
com cobrança por uso, progressão de clientes e custos comerciais. Ver
`progress/session-26.md`. — Session 25 — custo por dia do `/farm/profit` corrigido
em prod (ExPlugins 1,00 → 1,80, valor do seed nunca atualizado; 8,40 → 13,20/dia
com 6 bots) + tooltip que abre o custo linha a linha + card de payback do bot +
editor de custos que simula sem persistir + preço manual do comprador. Ver
`progress/session-25.md`. — Anterior: session 23 continuação — limpeza de frontend +
...
| Session | Date | Theme |
|---------|------|-------|
| [26](progress/session-26.md) | 2026-08-21 | Simulador comercial em `/farm/bot-sales`: dois modos de cobrança por uso, rampa e cap de clientes, custos comerciais, margem, break-even e trilho diário de lucro |
| [25](progress/session-25.md) | 2026-08-09 | O custo por dia do `/farm/profit` mentia — com 6 bots o card mostrava US$ 8,40 porque o `explugins_key_cost_daily` da config padrão em prod ainda era **1,0000 (valor do seed)** em vez de 1,80; a multiplicação por bot sempre esteve certa. `UPDATE` aplicado direto no Postgres de prod (as 24 simulações têm snapshot próprio, nenhuma afetada); custo real com 6 bots: **US$ 13,20/dia**. Para o número nunca mais ficar opaco: `breakdownDailyCost()` + tooltip que abre o custo linha a linha (`6 × US$ 1,80 ExPlugins…`) com invariante testado contra `dailyCostFor`. Card novo **"Payback do bot"** (`lib/bot-payback.ts`): setup ÷ lucro por bot ao preço de hoje, **excluindo custos globais** (senão ligar outro bot pioraria o payback do primeiro). **Editor de custos só da tela** (`profit-cost-editor.tsx` + `use-profit-cost-override.ts`) — simula sem persistir, trocar de config descarta. Terceira base de preço `Preço do comprador (manual)` porque `sales` está vazia e o CNL paga 0,042 (**−34,6% vs mediana da G2G**); o ajuste % some no modo manual pra não contar o desconto duas vezes. Achado: um bot precisa de **~52 divines/dia** só pra cobrir o próprio custo ao preço do CNL. Known issues: a fórmula do custo diário está reimplementada em 3 lugares (a de `daily-breakdown.tsx` ignora o desconto do ExPlugins) e os labels "Explugins/DPB Key (diario)" não avisam que são por bot |
| [24](progress/session-24.md) | 2026-08-08 | Preço da concorrência (G2G) substitui o scraper do Discord — novo coletor (`lib/g2g-{client,stats,collector}.ts`, `scripts/g2g-price-collector/`, cron `Dockerfile.collector` */30) gravando em `g2g_price_snapshots`; `GET/POST /api/prices/g2g`; `/farm/prices` e KPI do dashboard repontados pra G2G (USD, concorrência — semântica diferente do antigo BRL de venda própria). Removido o pipeline do Discord: migration dropa `price_entries` (~31.8k msgs), `discord_sources` e o enum `Currency`; ~25 arquivos deletados; dep órfã `@google/genai` e 4 env vars fora. **`daily_prices` PRESERVADA** como arquivo read-only (~926 dias) porque `import-prices`/`create-projected`/overlay de cenários dependem dela. Descobertas da API do G2G: `q=<item>` é obrigatório na prática (sem ele `sort=lowest_price` nunca alcança o Divine) e **a G2G não guarda histórico de preço** — cada coleta perdida é buraco permanente. Filtro de outlier por MAD (k=4), não IQR: a cauda real tinha US$ 999,99 contra mediana de US$ 0,06. Bug pego em teste: `clampLimit(null)` devolvia 1. Segurança: `.gitignore` não cobria `.env.bak-*` (segredos versionáveis) → `.env*` + exceções. Agendamento em prod é **Scheduled Task do Coolify** (`*/30`) chamando a rota com `CRON_SECRET`, não container — o compose só vale em dev. **Migrations já aplicadas na VPS** via `docker exec` (o túnel não serve: `poth_app` não tem DDL e o `poe` só aceita conexão da rede docker), com backup de 34.575 linhas em `_backups/`; coleta real validada em prod. 746 testes passando, build limpo. **Pendente: commit + deploy** (a task só funciona depois que o `CRON_SECRET` entrar no container) |
...
| [17](progress/session-17.md) | 2026-04-25 | Operations control center + Reddit Trending Keywords — `/admin/operations` página unificada pra disparar todos os crawls/pipelines/jobs do engine com live status (Reddit/Ninja/KeyBERT) + tabela de Recent Pipeline Runs via `/seo/cron/runs`; Reddit Dashboard "Trending Topics" virou "Trending Keywords" consumindo `KeywordOpportunity source=reddit` (gate LLM universal) em vez de cluster de frases client-side |
