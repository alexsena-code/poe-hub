...
export default function PeoplePage() {
  ...
  const [editForm, setEditForm] = useState<{ name: string; aliases: string; role: string; category: string; relevance: string }>({
  ...
  if (loading) {
    return <div className="p-6 text-center text-muted-foreground text-sm">Loading people...</div>;
  }
      ...
      <div className="flex-1 min-h-0 overflow-auto scrollbar-none">
        <div className="overflow-x-auto border border-border rounded-lg">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-muted-foreground">No people found.</td></tr>
              ) : (
                filtered.map(p => (
                  editingName === p.name ? (
                    <tr key={p.id} className="border-b border-border/50 bg-surface">
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex gap-1 justify-end">
                          <button onClick={handleSave} disabled={saving} className="px-2 py-0.5 text-[10px] bg-emerald-600 hover:bg-emerald-500 text-white rounded disabled:opacity-40">
                            {saving ? '...' : 'Save'}
                          </button>
                          <button onClick={cancelEdit} className="px-2 py-0.5 text-[10px] bg-surface-hover text-muted-foreground rounded">
                            Cancel
                          </button>
                        </div>
                      </td>
                    ...
                    <tr key={p.id} className="border-b border-border/50 hover:bg-surface-hover transition-colors cursor-pointer" onDoubleClick={() => startEdit(p)}>
                      <td className="px-3 py-2 font-medium text-foreground">{p.name}</td>
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-1">
                          {p.aliases.map(a => (
                            <span key={a} className="text-[10px] px-1.5 py-0.5 rounded bg-foreground/5 text-muted-foreground">{a}</span>
                          ))}
                      ...
                      <td className={`px-3 py-2 text-xs ${relevanceColor(p.relevance)}`}>{p.relevance}</td>
                      <td className="px-3 py-2 text-right">
                        <div className="flex gap-1 justify-end">
                          <button
                            onClick={() => startEdit(p)}
                          ...
                          {confirmDelete === p.name ? (
                            <>
                              <button onClick={() => handleRemove(p.name)} className="px-2 py-0.5 text-[10px] bg-red-600 text-white rounded">Yes</button>
                              <button onClick={() => setConfirmDelete(null)} className="px-2 py-0.5 text-[10px] bg-surface-hover text-muted-foreground rounded">No</button>
                            </>
                          ) : (
                            <button
