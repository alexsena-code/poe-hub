...
import { ExternalLink, RefreshCw, ArrowUpDown, Percent } from "lucide-react";
...
// Discount tier coloring: 70%+ é a zona "raríssima" (emerald strong),
// 50-69% forte (success), 30-49% bom (warning), <30% marginal (neutral).
function discountColor(d: number) {
  if (d >= 70) return "text-success font-bold";
  if (d >= 50) return "text-success";
  if (d >= 30) return "text-warning";
  return "text-muted-foreground";
}

export function AlertsClient({ initialDeals, initialItems, initialHours }: AlertsClientProps) {
  ...
  async function fetchAll(h: string) {
    setLoading(true);
    try {
      const [dealsRes, itemsRes] = await Promise.all([
        fetch(`/api/hardware/deals?hours=${h}`),
      ...
      <Card>
        <CardContent className="pt-4 pb-4 space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={() => fetchAll(hours)} disabled={loading}>
