...
interface KeybertClientProps {
  initialStatus: KeybertStatus;
}

export function KeybertClient({ initialStatus }: KeybertClientProps) {
  const [online, setOnline] = useState(initialStatus.workerOnline);
  const [history, setHistory] = useState<TaskEntry[]>(initialStatus.taskHistory);
  const [dispatching, setDispatching] = useState(false);
  const [selectedModules, setSelectedModules] = useState<string[]>(ALL_MODULES.map((m) => m.id));
  // Custom inline toast — avoids import conflict with sonner in this isolated island
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      // silent — stale data shown until next poll
    }
  }, []);

  // 10s polling to track running tasks without page reload
  useEffect(() => {
  ...
  async function handleDispatch() {
    ...
    try {
      ...
      const data = await res.json();
      showToast(data.dispatched ? 'Task enviada ao worker' : 'Worker offline — task nao enviada');
      await loadStatus();
    } catch {
        ...
        {history.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {[...history].reverse().map((entry) => (
              <div key={entry.task.id} className="rounded-lg border border-border bg-surface p-4">
